...
public class DBClient {
    ...
    try {
      ...
      if (hasUnreturnedEquipment(memberNumber)) {
        ...
        while (unreturnedItems.next()) {
          ...
          System.out.println(
            "Item number: " + itemNumber + " | Quantity: " + loanedQuantity
          );
      ...
      String deleteEnrollmentQuery = String.format(
        "delete from umidmuzrapov.enrollment where memberNumber = %d",
        memberNumber
      );
      dbconn.createStatement().executeUpdate(deleteEnrollmentQuery);

      // Delete records from the EquipmentLoan table
      String deleteEquipmentLoanQuery = String.format(
        "delete from umidmuzrapov.equipmentloan where memberItem = %d",
        memberNumber
      );
      ...
      String findTransactionsQuery = String.format(
        "select transactionNumber from umidmuzrapov.transaction where memberNumber = %d",
        memberNumber
      );
      ResultSet transactions = dbconn
        .createStatement()
        .executeQuery(findTransactionsQuery);
      while (transactions.next()) {
        int transactionNumber = transactions.getInt("transactionNumber");

        // Delete linked records from the Purchase table
        String deletePurchaseQuery = String.format(
          "delete from umidmuzrapov.purchase where transactionNumber = %d",
          transactionNumber
        );
      ...
      String deleteTransactionQuery = String.format(
        "delete from umidmuzrapov.transaction where memberNumber = %d",
        memberNumber
      );
      dbconn.createStatement().executeUpdate(deleteTransactionQuery);

      // Finally, delete the member
      String deleteMemberQuery = String.format(
        "delete from umidmuzrapov.member where memberNumber = %d",
        memberNumber
      );
      int rowsAffected = dbconn
        .createStatement()
        .executeUpdate(deleteMemberQuery);
  ...
  private boolean isParticipatingInCourses(int memberNumber) {
    try {
      String query = String.format(
        "select count(*) from umidmuzrapov.enrollment where memberNumber = %d",
        memberNumber
      );
      Statement statement = dbconn.createStatement();
  ...
  public boolean addCourse(
    ...
    try {
      ...
      if (trainerNumber < 0) {
        ...
        System.out.printf(
          "Trainer with id: %d was chosen for this course.\n",
          trainerNumber
        );
      }
      ...
      if (rowsAffected > 0) {
        for (List<Integer> schedule : schedules) {
          String scheduleInsert =
            "INSERT INTO umidmuzrapov.Schedule (className, startDate, day, hour, minute, duration) values (?, ?, ?, ?, ?, ?)";
  ...
  private int getTrainerNumber(List<List<Integer>> schedules)
    ...
    Statement statement = dbconn.createStatement();
    String queryOne =
      "(SELECT t.trainerNumber FROM umidmuzrapov.Trainer t)" +
      " MINUS" +
    ...
    if (result.next()) {
      ...
      queryTwo.append(
        "(SELECT DISTINCT t.trainerNumber FROM umidmuzrapov.Trainer t) " +
      ...
      for (List<Integer> schedule : schedules) {
        String busySchedule =
          "OR (s.day = day? AND s.hour = hour? AND s.minute = minute?) ";
        busySchedule =
      ...
      String finalQuery = queryTwo
        .toString()
        .replace("?busy", busy.toString().replaceFirst("OR", ""));
  ...
  public boolean deleteCourse(String className, Date startDate) {
    try {
      ...
      String deleteCourseFromPackage =
        "delete from umidmuzrapov.CoursePackage where className=? and startDate=?";
      PreparedStatement deleteCourseFromPackageStm = dbconn.prepareStatement(
        deleteCourseFromPackage
      );
      deleteCourseFromPackageStm.setString(1, className);
      deleteCourseFromPackageStm.setDate(
        2,
      ...
      String deleteCourseFromSchedule =
        "delete from umidmuzrapov.Schedule where className=? and startDate=?";
      PreparedStatement deleteCourseFromScheduleStm = dbconn.prepareStatement(
        deleteCourseFromSchedule
      );
      deleteCourseFromScheduleStm.setString(1, className);
      deleteCourseFromScheduleStm.setDate(
        2,
        new java.sql.Date(startDate.getTime())
      );
  ...
  public List<String[]> listCoursesInPackage(String packageName) {
    ...
    String query =
      "SELECT className, startDate FROM umidmuzrapov.CoursePackage WHERE packageName = ?";
   ...
   */
  public boolean removeCourseFromPackage(
    String packageName,
    String className,
    String startDate
  ) {
    String deleteQuery =
      "DELETE FROM umidmuzrapov.CoursePackage WHERE packageName = ? AND className = ? AND startDate = ?";
   ...
   */
  public boolean addCourseToPackage(
    String packageName,
    String className,
    String startDate
  ) {
   ...
   *         otherwise.
   */
  public boolean isCourseInPackage(
    String packageName,
    String className,
    String startDate
  ) {
    String query =
      "SELECT COUNT(*) FROM umidmuzrapov.CoursePackage WHERE packageName = ? AND className = ? AND startDate = ?";
  ...
  public List<String[]> listOngoingCourses() {
    ...
    try {
      String query =
        "select * from umidmuzrapov.course where endDate > CURRENT_DATE";
      ...
      while (resultSet.next()) {
        ...
        ongoingCourses.add(
          new String[] { className, startDate.toString(), endDate.toString() }
        );
      }
    } catch (SQLException e) {
      e.printStackTrace();
  ...
  public boolean addPackage(String packageName, int packagePrice) {
    String insertPackageQuery =
      "INSERT INTO umidmuzrapov.Package (packageName, cost) VALUES (?, ?)";
  ...
  public boolean addCoursePackage(
    ...
    String checkPackageQuery =
      "SELECT COUNT(*) FROM umidmuzrapov.Package WHERE packageName = ?";
    String checkCourseQuery =
      "SELECT COUNT(*) FROM umidmuzrapov.Course WHERE className = ? AND startDate = ?";
    String insertQuery =
      "INSERT INTO umidmuzrapov.CoursePackage (packageName, className, startDate) VALUES (?, ?, ?)";
    ...
    try {
      ...
      try (
        PreparedStatement checkPackageStmt = dbconn.prepareStatement(
          checkPackageQuery
        )
      ) {
      ...
      try (
        ...
        for (String[] course : selectedCourses) {
          ...
          try (
            PreparedStatement checkCourseStmt = dbconn.prepareStatement(
              checkCourseQuery
            )
          ) {
            ...
            if (rs.next() && rs.getInt(1) == 0) {
              throw new SQLException(
                "Course not found: " + course[0] + ", " + course[1]
              );
            }
          }
  ...
  public boolean deleteCoursePackage(String packageName) {
    String deleteCoursePackageQuery =
      "DELETE FROM umidmuzrapov.CoursePackage WHERE packageName = ?";
    String deletePackageQuery =
      "DELETE FROM umidmuzrapov.Package WHERE packageName = ?";
  ...
  public List<String[]> queryOne() {
    ...
    try {
      ...
      String query =
        "select m.fname, m.lname, m.phoneNumber, " +
      ...
      while (resultSet.next()) {
        ...
        membersWithNegativeBalance.add(
          new String[] { firstName + " " + lastName, phoneNumber, balance }
        );
      }
    } catch (SQLException e) {
      e.printStackTrace();
  ...
  public List<String[]> queryThree() {
    ...
    try {
      // Fetch schedule data
      String query =
        "select t.trainerNumber, t.fname, t.lname, s.day, s.hour, s.minute, s.duration, c.startDate, c.endDate " +
   ...
   */
  private Map<String, Integer> calculateTotalHours(
    ...
    for (String[] entry : scheduleData) {
      ...
      trainerTotalHours.put(
        trainerKey,
        trainerTotalHours.getOrDefault(trainerKey, 0) + totalHours
      );
    }
  ...
  private static int[] countDaysOfWeek(
   ...
   */
  public void printCourseSchedule(String firstname, String lastname)
    throws SQLException {
    ...
    String query = String.format(
      "SELECT c.className, c.startDate, s.day, s.hour, s.minute, s.duration" +
    ...
    while (result.next()) {
      System.out.printf(
        "Course %s. %s, %d:%d. Duration: %d.\n",
        result.getString("className"),
        intToDay(result.getInt("day")),
        result.getInt("hour"),
   ...
   */
  public int memberExists(String firstname, String lastname)
    throws SQLException {
    Statement statement = dbconn.createStatement();
    String query =
      "SELECT * FROM umidmuzrapov.Member" + " WHERE fname='1?' and lname='2?'";
    ...
    if (memberNumbers.size() == 0) {
      ...
      System.out.println(
        "There are several numbers with this name. Ask the member for id and check." +
  ...
  public List<String[]> listAllPackagesAndCourses() {
    ...
    try {
      String query =
        "select p.packageName, p.cost, LISTAGG(cp.className, ', ') within group (order by cp.className) as courses " +
      ...
      while (resultSet.next()) {
        ...
        packageDetails.add(
          new String[] { packageName, Double.toString(cost), courses }
        );
      }
    } catch (SQLException e) {
      e.printStackTrace();
   ...
   */
  public String packagePurchased(
    String packageName,
    String cost,
    int memberNumber
  ) {
    try {
      ...
      String maxTransactionNumberQuery =
        "select MAX(transactionNumber) from umidmuzrapov.transaction";
      Statement maxTransactionNumberStmt = dbconn.createStatement();
      ResultSet rs = maxTransactionNumberStmt.executeQuery(
        maxTransactionNumberQuery
      );
      ...
      String purchaseQuery = String.format(
        "insert into umidmuzrapov.purchase (packageName, transactionNumber) VALUES ('%s', %d)",
        packageName,
        nextTransactionNumber
      );
      ...
      // enrollment table
      String upcomingCoursesQuery = String.format(
        "select cp.className, cp.startDate from umidmuzrapov.coursepackage cp " +
        "join umidmuzrapov.course c on cp.className = c.className and cp.startDate = c.startDate " +
        "where cp.packageName = '%s' AND c.startDate >= CURRENT_DATE",
        packageName
      );

      Statement upcomingCoursesStmt = dbconn.createStatement();
      ResultSet upcomingCourses = upcomingCoursesStmt.executeQuery(
