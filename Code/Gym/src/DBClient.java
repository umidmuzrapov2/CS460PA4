...
public class DBClient {
	...
	public String deleteMember(int memberNumber) {
		try {
			...
			if (hasNegativeBalance(memberNumber)) {
	            System.out.println("Member has a negative balance and cannot be deleted.");
	            dbconn.rollback(); // Rollback transaction
	            return "Cannot delete member due to negative balance.";
	        }
	 ...
	 * 
	 * Logic: - Executes SQL query to get borrowed equipment data - Prints out
	 * equipment name, quantity, borrowed date and returned date
	 *
	...
	public void printBorrowedEquipmentInfo(int memberNumber) {
		try {
			...
			String query = "SELECT e.descriptionComment AS equipmentName, el.quantity, el.borrowedDate, el.returnedDate "
					+ "FROM Equipment e " + "JOIN EquipmentLoan el ON e.itemNumber = el.itemNumber "
					+ "WHERE el.memberItem = ?";
			...
			try (PreparedStatement preparedStatement = dbconn.prepareStatement(query)) {
				...
				while (resultSet.next()) {
					...
					System.out.println("Equipment: " + equipmentName + ", Quantity: " + quantity + "Borrowed date: "
							+ borrowedDate.toGMTString() + ", Returned date: " + returnedDate.toGMTString());
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	/**
	 * Method hasNeagativeBalance
	 *
	 * Purpose: find if the member has negative balance
	 *
	 * Pre-condition: valid memberNumber
	 *
	 * Post-condition: None
	 *
	 * @param memberNumber
	 * @return Return true if negative balance; otherwise, return false
	 */
	private boolean hasNegativeBalance(int memberNumber) {
	    try {
	        String query = String.format(
	            "select m.fname, m.lname, m.phoneNumber, " +
	            "sum(case when t.type = 'deposit' then t.total ELSE -t.total end) as balance " +
	            "from umidmuzrapov.member m join umidmuzrapov.transaction t on m.memberNumber = t.memberNumber " +
	            "where m.memberNumber = %d " +
	            "group by m.memberNumber, m.fname, m.lname, m.phoneNumber " +
	            "having SUM(case when t.type = 'deposit' then t.total else -t.total END) < 0",
	            memberNumber
	        );
	        ResultSet resultSet = dbconn.createStatement().executeQuery(query);
	        return resultSet.next(); // True if a negative balance exists
	    } catch (SQLException e) {
	        e.printStackTrace();
	    }
	    return false;
