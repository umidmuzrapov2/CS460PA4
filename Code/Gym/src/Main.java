...
public class Main {
  ...
  private static void addMember(Scanner scanner, DBClient client) {
    ...
    if (result.startsWith("Member added successfully")) {
      ...
      String transaction = client.packagePurchased(
        packages.get(packageChoice - 1)[0],
        packages.get(packageChoice - 1)[1],
        memberNumber
      );
      System.out.println("\n" + transaction);
    }
        ...
        if (queryThreeResults.size() == 0) {
          ...
          for (String[] trainer : queryThreeResults) {
            ...
            System.out.println(
              ...
              " | Duration: " +
              trainer[5] +
              " minutes | Total Hours in December: " +
              trainer[6]
            );
          }
        }
  ...
  private static void addCoursePackage(Scanner scanner, DBClient client) {
    ...
    System.out.println(
      "Select courses to add to the package (Enter the number of the course, 'done' to finish):"
    );
    for (int i = 0; i < allCourses.size(); i++) {
      String[] course = allCourses.get(i);
      System.out.println(
        (i + 1) + ". " + course[0] + " - Start Date: " + course[1]
      );
    }
   ...
   */
  private static void editSelectedPackage(
    Scanner scanner,
    DBClient client,
    String packageName
  ) {
   ...
   *                    being added.
   */
  private static void addCourseToPackage(
    Scanner scanner,
    DBClient client,
    String packageName
  ) {
    System.out.println(
      "Select a course to add to the package '" + packageName + "':"
    );
    List<String[]> allCourses = client.listOngoingCourses();
    for (int i = 0; i < allCourses.size(); i++) {
      String[] course = allCourses.get(i);
   ...
   *                    being removed.
   */
  private static void removeCourseFromPackage(
    Scanner scanner,
    DBClient client,
    String packageName
  ) {
    System.out.println(
      "Select a course to remove from the package '" + packageName + "':"
    );
    ...
    if (courseIndex >= 0 && courseIndex < packageCourses.size()) {
      ...
      if (
        client.removeCourseFromPackage(
          packageName,
          selectedCourse[0],
          selectedCourse[1]
        )
  ...
  private static List<List<Integer>> getCourseSchedule() {
    ...
    System.out.println(
      "The format is 'Day, hour, minute, duration'.\ne.g.Monday, 15, 30, 50."
    );
  ...
  private static List<Integer> parseSchedule(String line) {
    ...
    for (int i = 1; i < 4; i++) {
      ...
      if (numericVal >= 0) {
        ...
        System.out.println(
          "String given when integer was expected." + words[i]
        );
        return null;
      }
    }
